    ...
    def decode(self, token_ids):
        ...
        for token_id in token_ids:
            ...
            if token in spec_tokens:
                continue
            tokens.append(token)
        # 拼接字符串,中间不是用空格拼接，而是直接拼接，中间没有什么空格
        return ''.join(tokens)
...
# 加载数据集
with open(settings.DATASET_PATH, 'r', encoding='utf-8') as f:#以读的方式打卡
    lines = f.readlines()
    # 将冒号统一成相同格式
...
# 统计词频，词频统计器，对update函数里面每一个出现的元素统计出现的次数。
counter = Counter()
for line in poetry:
    counter.update(line)
# 过滤掉低频词，得到的_token_s是由元组组成的列表，元素是(token,count)
_tokens = [(token, count) for token, count in counter.items() if count >= min_word_frequency]
# 按词频排序,key是排序的维度，表示按照元组中第一个元素进行排序，也就是按照count进行排序。
_tokens = sorted(_tokens, key=lambda x: -x[1])
# 去掉词频，只保留词列表，出现频次的高低，按照索引进行排序，也就是索引越靠前，表示出现的次数越多。
_tokens = [token for token, count in _tokens]

# 将特殊词和数据集中的词拼接起来，低频词已经默认使用UNK来代替了
...
class PoetryDataGenerator:
    ...
    def __init__(self, data, random=False):
        ...
        # 每个epoch迭代的步数,math.floor函数用于向下取整。很显然，这个不能四舍五入
    ...
    def __iter__(self):
        ...
        for start in range(0, total, self.batch_size):
            ...
            # yield x,y，这里只对词汇进行编码，从1开始说明不对开始符号进行读热处理，词汇量大小确定保证普及到每个词汇。
            #这里我们返回的是数据以及数据对应的独热向量，独热向量用于进行训练。之后记得删除我们的数据，防止占用过多的内存。
    ...
    def for_fit(self):
        ...
        创建一个生成器，用于训练
        """
        # 死循环，当数据训练一个epoch之后，重新迭代数据，表示不断生成数据。
        while True:
            # 委托生成器
